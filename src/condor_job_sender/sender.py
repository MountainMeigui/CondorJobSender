...
def configure(config_path):
    """Load cluster-specific settings from a YAML configuration file."""
    global directories, parameters
    global log_dir, error_dir, output_dir, exec_dir, python_path
...
def _require_configuration():
    if directories is None:
        raise RuntimeError(
            "Condor Job Sender has not been configured. "
            "Call configure('configurations.yml') first."
        )
...
def _require_htcondor():
    if htcondor is None:
        raise RuntimeError(
            "HTCondor's Python bindings are unavailable. "
            "Run job submission on Linux, through WSL, or on your "
            "cluster's Linux login machine."
        )
